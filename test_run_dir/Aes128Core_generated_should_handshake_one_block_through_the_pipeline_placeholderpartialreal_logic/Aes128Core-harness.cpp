...
#endif
#if VM_COVERAGE
  VerilatedCov::write(R"(/home/crypto-chisel-llm/test_run_dir/Aes128Core_generated_should_handshake_one_block_through_the_pipeline_placeholderpartialreal_logic/coverage.dat)");
#endif
  top->final();
  // TODO: re-enable!
  // delete top;
...
struct sim_state {
  ...
  inline void poke(int32_t id, int64_t value) {
    ...
    switch(id) {
      ...
      case 3 : dut->io_out_ready = u; break;
  ...
  inline int64_t peek(int32_t id) {
    ...
    switch(id) {
      ...
      case 3 : value = dut->io_out_ready; break;
      case 5 : value = dut->io_in_ready; break;
  ...
  inline void poke_wide(int32_t id, int32_t offset, int64_t value) {
    ...
    size_t words = 0;
    switch(id) {
      case 2 : data = dut->io_in_bits_bits; words = 4; break;
      case 4 : data = dut->io_key_bits; words = 4; break;
  ...
  inline int64_t peek_wide(int32_t id, int32_t offset) {
    ...
    switch(id) {
      case 2 : data = dut->io_in_bits_bits; words = 4; break;
      case 4 : data = dut->io_key_bits; words = 4; break;
      case 7 : data = dut->io_out_bits_bits; words = 4; break;
...
static sim_state* create_sim_state() {
  ...
  std::string dumpfile = R"(/home/crypto-chisel-llm/test_run_dir/Aes128Core_generated_should_handshake_one_block_through_the_pipeline_placeholderpartialreal_logic/Aes128Core.)";
